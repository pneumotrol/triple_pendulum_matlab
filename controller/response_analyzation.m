...
%% read data (Simulink)
load("data/simout_state_feedback.mat");

sim_time = position.Time;
sim_position = position.Data(:,1:dim);
sim_effort = position.Data(:,dim+1);

%% read data (ROS2)
t = readcell("data/rosout_state_feedback.csv");
...
plot(sim_time, sim_effort(:,1));
plot(ros2_time, ros2_effort(:,1));
xlabel("time (s)");
ylabel("effort (N)");
xlim([0, 5]);
ylim([-20.0, 20.0]);
